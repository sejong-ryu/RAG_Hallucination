...
def find_subsequence(main_seq, sub_seq):
    for i in range(len(main_seq) - len(sub_seq) + 1):
        if torch.equal(main_seq[i:i+len(sub_seq)], torch.tensor(sub_seq, device=main_seq.device)):
            return i
    return -1
...
def evaluate_em(results, gold_ans, pred_ans):
    em_score = 0
    correct, wrong = [], []

    for res in results:
        ground_truths = res[gold_ans]
        prediction = res[pred_ans]
...
def evaluate_f1(results, gold_ans, pred_ans):
    f1_total = 0
    correct, wrong = [], []
