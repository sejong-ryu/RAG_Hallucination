...
class Model:
    ...
    def generate(self,question: str, context: str, gen_max_length: int = 10, use_rag_hallu=True, numk=3):
        ...
        with torch.no_grad():
            ...
            candidate_answers = []
            for i in range(numk):
                input_ids_i = input_ids.clone()
                attention_mask_i = attention_mask.clone()
                tokens = []
                log_probs = []
                
                ## Input first token & log prob of i-th top-k
                first_token = topk_tokens[0, i].unsqueeze(0) 
                tokens.append(first_token)
                log_probs.append(torch.log(topk_probs[0, i]).item())
                
                input_ids_i = torch.cat([input_ids_i, first_token.unsqueeze(0)], dim=-1)
                attention_mask_i = torch.cat([attention_mask_i, torch.tensor([[1]], device=self.device)], dim=-1)
                
                ## Generate in greedy manner
                for cur_len in range(gen_max_length-1):
                    outputs = self.model(input_ids=input_ids_i, attention_mask=attention_mask_i, output_attentions=True, return_dict=True, use_cache=True)
                    next_token_logits = outputs.logits[:, -1, :]
                    probs = F.softmax(next_token_logits, dim=-1)  # (1, vocab_size)
                    next_token = torch.argmax(probs, dim=-1)
                    next_token_log_prob = torch.log(probs[0, next_token]).item()
                    ...
                    input_ids_i = torch.cat([input_ids_i, next_token.unsqueeze(0)], dim=-1)
                    attention_mask_i = torch.cat([attention_mask_i, torch.tensor([[1]], device=self.device)], dim=-1)
                    
                ## Decode the generated tokens
                generated_tokens = torch.cat(tokens)
                answer_text = self.tokenizer.decode(generated_tokens, skip_special_tokens=True)
                norm_ans = normalize_answer(answer_text)
                
                ## Calculate the avg log prob of the normalized answer
                #norm_ans_ids = self.tokenizer.encode(norm_ans, add_special_tokens=False)
                #start_idx = find_subsequence(generated_tokens, norm_ans_ids)
                
                #matched_log_probs = log_probs[start_idx:start_idx + len(norm_ans_ids)]
                #avg_log_prob = sum(matched_log_probs) / len(matched_log_probs)
                avg_log_prob = sum(log_probs) / len(log_probs)
                
                candidate_tokens.append(generated_tokens)
            ...
            if use_rag_hallu:
                ...
                new_input_ids = new_tokenized_inputs['input_ids']
                new_attention_mask = new_tokenized_inputs['attention_mask']
                
                new_tokens = []
                #new_log_probs = []
                
                for cur_len in range(50):
                    new_outputs = self.model(input_ids=new_input_ids, attention_mask=new_attention_mask, output_attentions=True, return_dict=True, use_cache=True)
                    new_next_token_logits = new_outputs.logits[:, -1, :]
                    new_probs = F.softmax(new_next_token_logits, dim=-1)
                    new_next_token = torch.argmax(new_probs, dim=-1)
                    
                    new_tokens.append(new_next_token)
                    #new_log_probs.append(torch.log(new_probs[0, new_next_token]).item())
                    
                    if new_next_token == self.tokenizer.eos_token_id:
                        break
                
                    new_input_ids = torch.cat([new_input_ids, new_next_token.unsqueeze(0)], dim=-1)
                    new_attention_mask = torch.cat([new_attention_mask, torch.tensor([[1]], device=self.device)], dim=-1)
                    
                #avg_new_log_prob = sum(new_log_probs) / len(new_log_probs)
                new_generated_tokens = torch.cat(new_tokens)
                new_sentence = self.tokenizer.decode(new_generated_tokens, skip_special_tokens=True)
                
                ### 2. Get avg_log_prob of after sentence for other candidates
                #new_candidate_log_probs = [avg_new_log_prob]
                ...
                for i, cand_ans in enumerate(candidate_answers):
                    ...
                    new_cand_input_ids = new_cand_tokenized_inputs['input_ids']
                    new_cand_attention_mask = new_cand_tokenized_inputs['attention_mask']
                    
                    total_log_prob = 0.0
                    for j in range(len(new_generated_tokens)):
                        new_cand_outputs = self.model(input_ids=new_cand_input_ids, attention_mask=new_cand_attention_mask, output_attentions=True, return_dict=True, use_cache=True)
                        new_cand_next_token_logits = new_cand_outputs.logits[:, -1, :]
                        new_cand_probs = F.softmax(new_cand_next_token_logits, dim=-1)
                        
                        tgt_token = new_generated_tokens[j].item()
                        total_log_prob += torch.log(new_cand_probs[0, tgt_token]).item()
                        
                        new_cand_input_ids = torch.cat([new_cand_input_ids, new_generated_tokens[j].unsqueeze(0).unsqueeze(0)], dim=-1)
                        new_cand_attention_mask = torch.cat([new_cand_attention_mask, torch.tensor([[1]], device=self.device)], dim=-1)
                        
                    avg_new_cand_log_prob = total_log_prob / len(new_generated_tokens)
                    new_candidate_log_probs.append(avg_new_cand_log_prob)
                
                ### 3. Get marginalized prob
                candidate_log_probs = torch.tensor(candidate_log_probs)  # log p(x | L)
                new_candidate_log_probs = torch.tensor(new_candidate_log_probs)  # log p(R | L, x)
